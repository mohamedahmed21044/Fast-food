...
import { prodectdetails } from '../ReduxTolkit/Slices'
...
const BergerDatails = () => {
  ...
  let {cart,setcart,option,setoption,optionone,setoptionone,optiontwo,setoptiotwo,optionthree,setoptionthree,optionfour,setoptionfour,optionfive,setoptionfive,optionsix,setoptiosix,bigcola,setbigcola,greenlemon,setgreenlemon,greencola,setgreencola,orangejuice,setorangejuice}=useContext(HandleText)
  ...
  useEffect(()=>{
    dispatch(prodectdetails(slug))
  },[])
  
  const AllProduct=[
    {price:Number(ProductDetails?.data?.attributes?.price),rate2: Number( ProductDetails?.data?.attributes?.rate2),rate1: Number(ProductDetails?.data?.attributes?.rate1),brownbread:5,opption:65}
  ]
...
const Allcart=[
  {img:ProductDetails?.data?.attributes?.img?.data[0]?.attributes?.url,title:ProductDetails?.data?.attributes?.title,description:ProductDetails?.data?.attributes?.discription,price:finalcalc(),id:Math.random(),quntity:1,img2:optionone==false? ProductDetails?.data?.attributes?.img2?.data[0]?.attributes?.url:ProductDetails?.data?.attributes?.img?.data[0]?.attributes?.url,Drinks:[bigcola?"Big Cola":"",greencola?"green cola":"",greenlemon?"green lemon":"",orangejuice?"orange juice":""],Des:[option?"Onion Rings Combo":"",optionthree?"Cheesy Fries Combo (French Fries)":"",optionsix?"Cheesy Fries Combo (Diablo Fries)":"",optiontwo?"French Fries Combo":"",optionfour?"Diablo Fries Combo":"",optionfive?"Blanco Fries Combo (French Fries)":""],beef:[beef?",Beef Bacon":""],cheese:[cheese?",Cheddar Cheese":""],piceles:[pickeles?",Pickles":""],mash:[mashroom?",Sautéed Mushroom":""]},
]
  ...
  return (
    ...
    <div className="product">
        ...
        <div className="Description">
            <h3>{ProductDetails?.data?.attributes?.title}</h3>
            <p>{ProductDetails?.data?.attributes?.discription}</p>
        </div>
    </div>
    <div className="size">
      <h4>size</h4>
      <div className="gm">
        ...
        {phototwo?<div className="photo-one" style={{marginTop:"18px"}} >
          <img src={img7} alt="" />
          <span>200 gm</span>
          <p>+EGP{ProductDetails?.data?.attributes?.rate1}</p>
        ...
        {photothree?<div className="photo-one" style={{marginTop:"18px"}} >
          <img src={img7} alt="" />
          <span>400 gm</span>
          <p>+EGP{ProductDetails?.data?.attributes?.rate2}</p>
      ...
      <div className="gm-res">
        <div className="frist" onClick={()=>(setphotoone(true),setphototwo(false),setphotothree(false))}>
          ...
          <p className={`${photoone?"label-color":""}`}>150 gm (EGP {ProductDetails?.data?.attributes?.price})</p>
        </div>
        <div className="second" onClick={()=>(setphotoone(false),setphototwo(true),setphotothree(false))}>
          ...
          <p className={`${phototwo?"label-color":""}`}> 200 gm (EGP {ProductDetails?.data?.attributes?.rate1})</p>
        </div>
        <div className="third" onClick={()=>(setphotoone(false),setphototwo(false),setphotothree(true))}>
          ...
          <p className={`${photothree?"label-color":""}`}> 400 gm (EGP {ProductDetails?.data?.attributes?.rate2})</p>
        </div>
    </div>
    </div>
